let taxSwitch1 = document.getElementById("switchCheckDefault1");
taxSwitch1.addEventListener('click', (event)=>{
...
let taxSwitch2 = document.getElementById("switchCheckDefault2");
taxSwitch2.addEventListener('click', (event)=>{
    if(event.target.checked){
        let prices = document.getElementsByClassName("prices");
        for(let price of prices){
            let amount = parseInt(price.innerText.split("/")[0].replace("₹","").replaceAll(",",""));
